...
def respond(voice_data):
  ...
  ******** if there_exists(["where is office room"]):
       greetings = ["located in admin block first floor"]
       greet = greetings[random.randint(0,len(greetings)-1)]
       speak(greet)
    
    if there_exists(["where is cse department "]):
       greetings = ["on circuit block third floor"]
       greet = greetings[random.randint(0,len(greetings)-1)]
       speak(greet)

   if there_exists(["where is exam cell"]):
